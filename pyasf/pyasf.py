...
class unit_cell(object):
    ...
    def write_cif(self, path):
        ...
        lines.append("#" + 72*"=")
        lines.append("#")
        lines.append("# Crystallographic Information File")
        lines.append("# Generated by pyasf")
        lines.append("# https://github.com/carichte/pyasf")
        lines.append("#")
        lines.append("#" + 72*"=")
        lines.append("")

        lines.append("data_generated_by_pyasf")
        lines.append("_audit_creation_date %s"%strftime("%Y-%m-%d"))

        rho = self.get_density()
        formula = " ".join(["%s%.2g"%item for item in self.stoichiometry.items()])
        lines.append("_chemical_formula_structural '%s'"%formula)
        lines.append("_chemical_formula_sum '%s'"%self.SumFormula)
        lines.append("_exptl_crystal_density_diffrn %g"%rho)
        lines.append("")

        lines.append(os.linesep)

        lines.append("_cell_length_a %g"%float(self.subs[self.a]))
        lines.append("_cell_length_b %g"%float(self.subs[self.b]))
        lines.append("_cell_length_c %g"%float(self.subs[self.c]))
        lines.append("_cell_angle_alpha %g"%np.degrees(float(self.subs[self.alpha] if self.alpha.is_Symbol else self.alpha)))
        lines.append("_cell_angle_beta  %g"%np.degrees(float(self.subs[self.beta]  if self.beta.is_Symbol else self.beta)))
        lines.append("_cell_angle_gamma %g"%np.degrees(float(self.subs[self.gamma] if self.gamma.is_Symbol else self.gamma)))
        lines.append("_cell_volume %g"%float(self.V.subs(self.subs)))
        lines.append("_cell_formula_units_Z %i"%min(self.multiplicity.values()))
        lines.append("_symmetry_space_group_name_H-M %s"%self.sg_sym)
        lines.append("_symmetry_Int_Tables_number %i"%self.sg_num)
        ...
        v = sp.Matrix(sp.symbols("x y z", real=True))
        for i, g in enumerate(self.generators):
            lines.append("%i "%(i+1) + "'%s'"%str(tuple((g[:,:3]*v + g[:,3]))).strip("()").replace(" ", "").replace(",", ", "))
        ...
        if self.Uaniso:
            ...
            lines.append("_atom_site_aniso_type_symbol")
            lines.append("_atom_site_aniso_B_11")
            lines.append("_atom_site_aniso_B_22")
            lines.append("_atom_site_aniso_B_33")
            lines.append("_atom_site_aniso_B_12")
            lines.append("_atom_site_aniso_B_13")
            lines.append("_atom_site_aniso_B_23")
            for label in self.AU_positions:
                if label not in self.Uaniso:
                    continue
                U = self.Uaniso[label]
                charge = self.charges[label]
                symbol = self.elements[label] + "%i"%abs(charge) + ["_", "+", "-"][np.sign(charge)]
